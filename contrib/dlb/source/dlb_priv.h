 ...
 */
#define DLB_SHM_SIZE                                                                               \
  (sizeof(dlb_shared_domain_t) + PAGE_SIZE + (PAGE_SIZE * MAX_NUM_LDB_PORTS) +                     \
   (PAGE_SIZE * MAX_NUM_DIR_PORTS) + ((2 * CACHE_LINE_SIZE) * MAX_NUM_LDB_PORTS) +                 \
   ((2 * CACHE_LINE_SIZE) * MAX_NUM_DIR_PORTS))

#define ROUND_UP_4KB(addr) (((uintptr_t)addr + (PAGE_SIZE - 1)) & ~(PAGE_SIZE - 1))

#define DLB_LDB_CQ_ADDR(base, id)                                                                  \
  (void*)((uintptr_t)base + ROUND_UP_4KB(sizeof(dlb_shared_domain_t)) + id * PAGE_SIZE)

#define DLB_DIR_CQ_ADDR(base, id) (void*)(DLB_LDB_CQ_ADDR(base, MAX_NUM_LDB_PORTS) + id * PAGE_SIZE)
...
#define VALIDATE_DLB_HANDLE(hdl) CHECK(((dlb_t*)hdl)->magic_num == DLB_MAGIC_NUM, EINVAL)
#define VALIDATE_DOMAIN_HANDLE(hdl)                                                                \
  CHECK(((dlb_domain_hdl_internal_t*)hdl)->magic_num == DOMAIN_MAGIC_NUM, EINVAL)
...
typedef struct dlb_port_hdl_internal {
  ...
  int (*wait_func)(struct dlb_port_hdl_internal* hdl, struct dlb_shared_port* port);
  int event_fd;
...
#define REFRESH_PORT_CREDITS(port_hdl, port, type)                                                 \
  port->credits[type].num = *port_hdl->popcount[type] - port->credits[type].pushcount;
...
#define PORT_CQ_IS_EMPTY(hdl, port) (hdl->cq_base[port->cq_idx].cq_gen != port->cq_gen)
 ...
 * expert users to provide their own memory space for CQ, PC, and NQ, and their
...
typedef struct {
  union {
    struct {
      ...
      uint16_t ldb_credit_low_watermark;
      /* Load-balanced credit quantum */
      uint16_t ldb_credit_quantum;
      /* Directed credit low watermark */
      uint16_t dir_credit_low_watermark;
      /* Directed credit quantum */
      uint16_t dir_credit_quantum;
    };
    struct {
      /* Credit low watermark */
      uint16_t credit_low_watermark;
      /* Credit quantum */
      uint16_t credit_quantum;
    };
  };
} dlb_create_port_adv_t;
